...
async visualizarFactura(ventaId: string) {
 ...
 async generarFacturaPDF(venta: any, config: any): Promise<Blob> {
  // 1. Importaciones dinámicas optimizadas para producción
  ...
  // Manejo de compatibilidad para el módulo QRCode en distintos entornos
  ...
  // --- ENCABEZADO CENTRAL (LETRA FACTURA) ---
  ...
  if (config.logo_url) {
    try {
      // Función auxiliar para convertir imagen a Base64 para evitar errores de CORS
      const base64Logo = await this.getBase64ImageFromURL(config.logo_url);
      ...
      console.warn("Error cargando logo, continuando sin imagen", e);
      doc.setFont('helvetica', 'bold');
